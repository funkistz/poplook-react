...
export default function DesktopCountDownBanner() {

    const { block } = useSelector((storeState: any) => storeState.block);
    const dispatch = useDispatch<ThunkDispatch<any, any, any>>();

    // Define
    const { listBanner } = usePage<any>().props;
    const scrollRef = useRef<HTMLDivElement>(null);
    const url = 'desktop_countdown_banner';
...

                        {
                            (block != null) ? (
                                <Box style={{ transition: 'width 0.2s' }} m={0} mt={10} mb={40}>
                                    <BlockCreator needCategory={true} />
                                </Box>
                            ) : (
                                <Flex bg={'#fff'} p={10}>
...
DesktopCountDownBanner.layout = (page: any) => <AdminLayout children={page} title='Desktop Countdown Banner Setting' />;
