...
class CountdownBanner extends Model
{
    ...
    protected $table      = 'countdown_banners';
    ...
    public static function getLiveList($type)
    {
        ...
        $live = CountdownBanner::activeLive()->where('type', $type)->first();

        if ($live) {
            $banners->push($live);
        }
        $active = CountdownBanner::active()->where('type', $type)->get();
    ...
    {
        $banners = CountdownBanner::where('active', '!=', 1)->where('type', $type)->orderBy('created_at', 'DESC')->get();
