...
class MobileMenuSettingController extends Controller
{
    public function mobile_preview()
    {
        // $response = Http::timeout(10)->get('/api/home_banner/desktop');
        $request = Request::create('/api/top_menu/mobile', 'GET');
        $response = Route::dispatch($request);
        $response = json_decode($response->getContent(), true);

        // dd($response);

        return view('preview_mobile_menu', ['data' => $response['data']]);
    }
    /**
    ...
    public function index()
    {
        ...
        $listActiveBanner = TopMenu::getLiveList('mobile');
        $listDraftBanner = TopMenu::getDraftList('mobile');
        ...
        return Inertia::render('Admin/MenuSetting/MobileIndex', [
